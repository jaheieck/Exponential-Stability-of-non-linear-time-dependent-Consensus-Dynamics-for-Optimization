...
def first_order_cbo_update_Euler_Maruyama(V, V_alpha, anisotropic, l, sigma, dt, device=None):
    device = torch.device('cpu') if device is None else device

    # generate same noise for all particles
    n, d = V.shape
    if device.type == 'cuda':
        noise = torch.cuda.FloatTensor(d).normal_(0, 1)
    else:
        noise = torch.FloatTensor(d).normal_(0, 1)
...
def first_order_cbo_update_tamed_Euler(V, V_alpha, anisotropic, l, sigma, dt, epoch, device=None):
    device = torch.device('cpu') if device is None else device
...
def first_order_cbo_update_advanced_Heun(V, V_alpha, anisotropic, l, sigma, dt, function, alpha, device=None):
    device = torch.device('cpu') if device is None else device
...
# INPUT: - function (function): optimization function f
#        - alpha (scalar): agent weight for CP
def first_order_cbo_update_simple_Heun(V, V_alpha, anisotropic, l, sigma, dt, function, alpha, device=None):
    device = torch.device('cpu') if device is None else device
...
# INPUT: - function (function): optimization function f
#        - alpha (scalar): agent weight for CP
def first_order_cbo_update_order_1(V, V_alpha, anisotropic, l, sigma, dt, function, alpha, device=None):
    device = torch.device('cpu') if device is None else device
